...
> 파이썬 객체를 **그대로 파일에 저장하고 다시 복원**하는 기능

일반 텍스트 파일은 문자열만 저장 가능하지만,  
`pickle`은 **리스트, 딕셔너리, 튜플, 객체까지 그대로 저장** 가능하다.
...
2. 보안 취약  
    → 신뢰되지 않은 pickle 파일은 load 하면 안됨
    

```python
pickle.load()  # 악성 코드 실행 가능
```
