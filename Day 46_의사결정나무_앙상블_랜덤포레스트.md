...
```
모델A → [class0: 0.7,  class1: 0.3]  ← class0 강하게 예측
모델B → [class0: 0.4,  class1: 0.6]  ← class1 약하게 예측
모델C → [class0: 0.6,  class1: 0.4]  ← class0 약하게 예측
         -------------------------------- 평균
          class0: 0.57  class1: 0.43
                 ↓
          최종 예측: class 0  (0.57 > 0.43)
```

**Hard Voting과 비교 — Soft Voting이 더 좋은 이유**

```
모델A → [class0: 0.51, class1: 0.49]  ← 아슬아슬하게 0 예측
모델B → [class0: 0.49, class1: 0.51]  ← 아슬아슬하게 1 예측
모델C → [class0: 0.90, class1: 0.10]  ← 매우 강하게 0 예측
         -------------------------------- 평균
          class0: 0.63  class1: 0.37
                 ↓
          최종 예측: class 0

Hard Voting이었다면 → 2:1로 0 (모델C의 확신 크기는 무시됨)
Soft Voting        → 확신이 강한 모델C의 의견이 더 많이 반영됨
```
